...
//
// Le script s'arrÃªte et reprend lÃ  oÃ¹ il en Ã©tait (resume automatique).
// Rapport final dans CONFIG.resultDir/pipeline_report.json
//
// ============================================================
// CONFIGURATION - Ã€ ADAPTER PAR SESSION
// ============================================================
...
var CONFIG = {
  ...
  // Filtres Ã  traiter (null = auto-dÃ©tection depuis les sous-dossiers)
  filters:           ["L"],
  ...
  // Stars  : rejette en dessous de P_stars  (ex: P10 = rejette les 10% moins Ã©toilÃ©es)
  // FWHM   : rejette au dessus de P_fwhm   (ex: P90 = rejette les 10% plus floues)
  // SNRWeight: rejette en dessous de P_snr (ex: P10 = rejette les 10% plus bruitÃ©es)
  pStars:            10,
  pFWHM:             90,
  pSNR:              10,

  // Subframe scale (arcsec/px - dÃ©pend de la camÃ©ra/optique)
  subframeScale:     2.26,

  // Filtre prÃ©fÃ©rÃ© pour la rÃ©fÃ©rence StarAlignment
...
// ============================================================
// PHASE 0 : DÃ©tection des filtres
// ============================================================
...
function findDOFMasters(filter, temp, expTime) {
  ...
  console.writeln("  DOF " + filter + " (T=" + tempRounded + "Â°C Exp=" + expInt + "s):" +
...
function runSubframeAndSSWEIGHT(filter) {
  ...
  console.writeln("  [" + filter + "] MÃ©triques: FWHM[" +
  ...
  console.writeln("  [" + filter + "] ApprouvÃ©s: " + approved.length + "/" + data.length + " (rejetÃ©s: " + rejected + ")");

  if (approved.length === 0) {
    console.writeln("  [" + filter + "] ERREUR: aucun fichier approuvÃ© - vÃ©rifier les paramÃ¨tres");
    return [];
  }

  // --- Calcul SSWEIGHT (sur set approuvÃ© uniquement) ---
  ...
  for (var a = 0; a < approved.length; a++) {
    ...
    // Chemin de sortie: _c_abe.xisf â†’ _c_abe_a.xisf
...
function runIntegration(filter) {
  ...
  // Sauvegarder la fenÃªtre rÃ©sultat
  var wins = ImageWindow.windows;
...
function main() {
  ...
  if (filters.length === 0) {
    ...
    console.writeln("ERREUR: aucun sous-dossier de filtre trouvÃ© dans " + CONFIG.rootDir);
    return;
  }
  ...
  if (allApproved.length === 0) {
    ...
    console.writeln("ERREUR: Aucun fichier approuvÃ©. VÃ©rifier les donnÃ©es source.");
    return;
  }

  // ---- SÃ©lection de la rÃ©fÃ©rence StarAlignment ----
  var refFile = null;
  // PrÃ©fÃ©rer le filtre dÃ©fini dans CONFIG
  ...
  console.writeln("RÃ©fÃ©rence StarAlignment: " + (refFile ? File.extractName(refFile) : "N/A"));
  ...
  console.writeln("");
  console.writeln("â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—");
  console.writeln("â•‘   PIPELINE COMPLETE - " + filters.join("+") + "        â•‘");
  console.writeln("â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•");
